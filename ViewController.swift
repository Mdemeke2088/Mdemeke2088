...
//  FlyForFun
//
//  Created by Mikyas Demeke on 3/22/24.
//
...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var lblMileageProgram: UILabel!
    @IBOutlet weak var lblName: UILabel!
    @IBOutlet weak var imgImage: UIImageView!
    @IBOutlet weak var SvBottom: UIScrollView!
    
    @IBOutlet weak var SvFav: UISwitch!
    
    @IBAction func SvFavValChanged(_ sender: Any) {
        if(SvFav.isOn)   {
    ...
    override func motionBegan(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        ...
        lblMileageProgram.alpha = 0
        SvBottom.alpha = 0
        lblDisclaimer.alpha = 0
    }
    override func motionEnded(_ motion: UIEvent.EventSubtype, with event: UIEvent?) {
        UIView.animate(withDuration: 3, animations: {
            ...
            self.lblMileageProgram.alpha = 1
            self.SvBottom.alpha = 1
    ...
    override func viewDidLoad() {
        ...
        let soundUrl = URL(fileURLWithPath:
                            Bundle.main.path(forResource: "Airplane", ofType: "mp3")!)
    ...
    @IBAction func btnNextAirline(_ sender: Any) {
        SetLebels()
    }
   
    @IBAction func btnWebsite(_ sender: Any) {
        let browserApp = UIApplication.shared
        let url = URL(string:ra.Website)
        browserApp.open(url!)
    }
    func SetLebels(){
        var randomAirways = FlyForFunObjectArray.randomElement()
            randomAirways = seguelFlyForFun
            ra = randomAirways!
            lblName.text = randomAirways?.Name
            lblDescription.text = randomAirways?.Description
            lblDestinations.text = randomAirways?.Destinations
            lblMileageProgram.text = randomAirways?.MileageProgram
            lblDisclaimer.text = randomAirways?.Disclaimer
        
        
            let fav = UserDefaults.standard.string(forKey: "Favorite")
            SvFav.isOn = (randomAirways?.Name == fav)
        
        switch (randomAirways?.MileageProgram){
        case "Star Alliance":
            lblMileageProgram.backgroundColor = UIColor(red:83/255, green: 36/355, blue: 111/255, alpha: 1)
        break
        case "One World":
            lblMileageProgram.backgroundColor = UIColor(red:235/255, green: 224/355, blue: 49/255, alpha: 1)
        break
        case "Sky Team":
            lblMileageProgram.backgroundColor = UIColor(red:62/255, green: 103/355, blue: 235/255, alpha: 1)
        break
        default:
            lblMileageProgram.backgroundColor = UIColor(red:47/255, green: 64/355, blue: 74/255, alpha: 1)
        }
            imgImage.image = UIImage(named: randomAirways!.Image)
            imgImage.layer.cornerRadius = 10
            imgImage.layer.borderColor = UIColor.black.cgColor
            imgImage.layer.borderWidth = 2
    
            lblDescription.layer.cornerRadius = 15
            lblDescription.layer.borderColor = UIColor.black.cgColor
            lblDescription.layer.borderWidth = 2
        
            lblDisclaimer.layer.cornerRadius = 12
            lblDisclaimer.layer.borderColor = UIColor.black.cgColor
            lblDisclaimer.layer.borderWidth = 2
        
            SvBottom.layer.cornerRadius = 15
            SvBottom.layer.borderColor = UIColor.black.cgColor
            SvBottom.layer.borderWidth = 2
